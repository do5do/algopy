...
=> sum()을 사용하면 5번 히든 테케에서 시간초과가 난다.
    ...
    - 어라.. sum()을 쓰지 않아도 5번 통과가 안된다.
    - bridge를 deque에서 list로 바꿔주고 pop(0)을 하니까 통과된다.. list의 pop(index)는 O(N)으로 알고 있는데 왜지?
    - => pop은 덱이 더 빠른게 맞지만, deque을 생성하는데 시간복잡도가 O(N)이라서 시간초과가 난 것 같다.
         deque을 생성하는 시간이 list에서 pop을 하는 시간보다 더 큰가보다..
'''