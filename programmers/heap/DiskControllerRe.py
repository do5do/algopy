...
- 평균을 가장 줄이는 방법은 요청이 가장 먼저 오는 것 중에 작업 시간이 짧은 순으로 진행한다.
    - 현재 처리 가능하면 힙에 담는다.
        - 현재 처리 가능하다는 것은 작업 요청이 이미 들어와 있는 것. 즉, 요청 시점이 현재 시점보다 작거나 같고 이전 작업의 시작 시간보다 커야한다.
        - 힙에 담을 때는 요청이 가장 먼저오는 것을 걸렀으니 작업 시간이 짧은 순으로 정렬해야 한다. (요청 시간이 같은 작업이 있을 수 있음)
    - 현재 시점에 처리할 수 있는 작업이 없으면 시간이 흘러야 한다. 플러스 1초
    - 처리할 수 있는 작업이 있을 때 now는 해당 작업이 종료된 시점이 되고, 소요 시간은 (작업 종료 시점 - 대기 시간(=요청 시점))이 된다.
